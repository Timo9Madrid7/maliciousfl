...
from Common.Utils.gradients_recorder import write_to_txt
...
class ClearDenseClient(WorkerBase):
    ...
    def update(self):
        if self.client_id < 1000:
            ...
            # malicious client when id>= 1000
             gradients = np.random.normal(0, 0.1, self._grad_len).tolist()

        # write gradients to csv
        write_to_txt("Eva/gradients/gradients_"+str(self.client_id)+'.txt', data=gradients)
